...
import { StandardizationType, StandardizationDasarType } from '../../standardization/types';
//------------------------------------------------------------------
...
const StandardizationProvider = ({children}: StandardizationProviderProps) => {
  ...
  const getStandardization = async(kppnId: string, period: number) => {
    try{
      ...
      const response = await axiosJWT.get(`/getStdWorksheet/${kppnId}?period=${period}&time=${time}&dasar=${dasar ? dasar : selectedDasar}`);
