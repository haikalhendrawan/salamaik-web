...
interface KanwilNoteCKProps {
  checklist: WsCKJunctionType;
  isPastDue: boolean;
}

export default function KanwilNoteCK({ checklist, isPastDue }: KanwilNoteCKProps) {
  const { auth } = useAuth();
  ...
  const saveNote = async () => {
    ...
    socket.emit(
      ...
      async (response: { success: boolean; message?: string }) => {
        try {
          if (!response?.success) {
            openSnackbar(response?.message || 'Gagal menyimpan catatan Kanwil CK', 'error');
            return;
          }
          ...
          openSnackbar(
            error instanceof Error ? error.message : 'Gagal memperbarui catatan Kanwil CK',
            'error'
          );
        } finally {
          setIsSaving(false);
    ...
    <Box sx={{ position: 'relative', width: '100%' }}>
      <TextField
        ...
        inputProps={{
          ...
          spellCheck: false,
          sx: { fontSize: 12 },
          'aria-label': `Catatan Kanwil untuk ${checklist.materi}`,
        }}
      />
      {(isSaving || isSyncing) && (
